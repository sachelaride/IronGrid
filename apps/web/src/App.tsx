...
const MonitoredDevices = lazy(() => import('./components/MonitoredDevices').then(m => ({ default: m.MonitoredDevices })));
const Dashboard = lazy(() => import('./components/Dashboard').then(m => ({ default: m.Dashboard })));
const InventoryManager = lazy(() => import('./components/InventoryManager').then(m => ({ default: m.InventoryManager })));
const TicketManager = lazy(() => import('./components/TicketManager').then(m => ({ default: m.TicketManager })));
const AlertCenter = lazy(() => import('./components/AlertCenter').then(m => ({ default: m.AlertCenter })));
const OrganizationHub = lazy(() => import('./components/OrganizationHub').then(m => ({ default: m.OrganizationHub })));
const InventoryReport = lazy(() => import('./components/InventoryReport').then(m => ({ default: m.InventoryReport })));
const Reports = lazy(() => import('./components/Reports').then(m => ({ default: m.Reports })));
const Settings = lazy(() => import('./components/Settings').then(m => ({ default: m.Settings })));
const UserManager = lazy(() => import('./components/UserManager').then(m => ({ default: m.UserManager })));
const NetworkManagementHub = lazy(() => import('./components/NetworkManagementHub').then(m => ({ default: m.NetworkManagementHub })));
const NetworkDiscovery = lazy(() => import('./components/NetworkDiscovery').then(m => ({ default: m.NetworkDiscovery })));
const AuditSyslogView = lazy(() => import('./components/AuditSyslogView').then(m => ({ default: m.AuditSyslogView })));
const GrafanaManager = lazy(() => import('./components/GrafanaManager').then(m => ({ default: m.GrafanaManager })));
const DeviceInventoryDetail = lazy(() => import('./components/DeviceInventoryDetail').then(m => ({ default: m.DeviceInventoryDetail })));
...
const SystemMaintenance = lazy(() => import('./components/SystemMaintenance').then(m => ({ default: m.SystemMaintenance })));
const MonitoringGraphs = lazy(() => import('./components/MonitoringGraphs').then(m => ({ default: m.MonitoringGraphs })));
const ToolsHub = lazy(() => import('./components/ToolsHub').then(m => ({ default: m.ToolsHub })));
const SLADashboard = lazy(() => import('./components/SLADashboard').then(m => ({ default: m.SLADashboard })));
const IPAMDashboard = lazy(() => import('./components/IPAMDashboard').then(m => ({ default: m.IPAMDashboard })));
const AlertSettings = lazy(() => import('./components/AlertSettings').then(m => ({ default: m.AlertSettings })));
const CustomMapsContainer = lazy(() => import('./components/custom-maps/CustomMapsContainer').then(m => ({ default: m.CustomMapsContainer })));
...
const SystemAbout = lazy(() => import('./components/SystemAbout').then(m => ({ default: m.SystemAbout })));
const GrafanaExportManager = lazy(() => import('./components/TopologySettings').then(m => ({ default: m.GrafanaExportManager })));
const ManualViewer = lazy(() => import('./components/ManualViewer').then(m => ({ default: m.ManualViewer })));
const ContributionPage = lazy(() => import('./components/ContributionPage').then(m => ({ default: m.ContributionPage })));
...
function App() {
    ...
    useEffect(() => {
        if (meQuery.data) {
            ...
            if (!localStorage.getItem('irongrid_token')) {
                setIsCheckingAuth(false);
            }
        }
    ...
    return (
        <>
                    ...
                    >
                        <Suspense fallback={
                            <div className="flex flex-col items-center justify-center h-full space-y-4">
                                ...
                                <p className="font-label-caps text-[10px] text-on-surface-variant uppercase tracking-[0.3em] animate-pulse">Carregando Módulo...</p>
                            </div>
                        }>
            ...
            {currentTab === 'grafana_maps' && (
                <div className="glass-panel p-10 border-white/5 bg-surface-container/95">
                    <GrafanaExportManager />
                </div>
            )}
