...
export function CustomMapEditor({ mapId, onBack }: CustomMapEditorProps) {
    ...
    const STATIC_ELEMENTS = [
        { type: 'cloud', label: 'Internet / Nuvem', icon: <NodeIcon type="cloud" size={28} /> },
        ...
        { type: 'wifi', label: 'WiFi / Sinal', icon: <NodeIcon type="wifi" size={28} /> },
        { type: 'shield', label: 'Segurança / Shield', icon: <NodeIcon type="shield" size={28} /> },
        { type: 'server', label: 'Servidor (Estático)', icon: <NodeIcon type="server" size={28} /> },
        { type: 'database', label: 'Banco de Dados', icon: <NodeIcon type="database" size={28} /> },
        { type: 'terminal', label: 'Terminal / PC', icon: <NodeIcon type="terminal" size={28} /> },
    ];
    ...
    return (
        <div ref={containerRef} className="h-full flex relative overflow-hidden bg-page">
            ...
            <div className="absolute top-4 left-4 right-4 z-20 flex justify-between items-start pointer-events-none">
                <div className="bg-card border border-border p-3 rounded-xl shadow-lg pointer-events-auto flex items-center gap-4">
                    <button
                        onClick={onBack}
                ...
                {/* Header Controls */}
                <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 p-1 bg-background border rounded-lg shadow-lg">
                    <AlignmentToolbar 
            ...
            <div className="absolute top-[80px] right-4 z-30 flex flex-col gap-3 pointer-events-auto">
                <div className="flex flex-col bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="px-3.5 py-1.5 bg-slate-900 text-white text-[11px] font-black rounded-full text-center shadow-xl border border-white/10">
                    {Math.round(zoom * 100)}%
                </div>
            </div>
                                ...
                                <div className="p-4 space-y-4">
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            ...
                                            <select 
                                                className="w-full bg-card border border-border rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all text-main"
                                    ...
                                    <div className="pt-3 border-t border-border flex justify-between gap-2">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 px-3 py-2 rounded-lg bg-page text-main text-[10px] font-bold hover:bg-page transition-colors"
                                        >
                                            Fechar
                                        </button>
                                    </div>
            ...
            <aside className={`absolute top-0 right-0 h-full w-80 glass-panel border-l border-white/5 flex flex-col transition-all duration-500 z-[150] bg-surface backdrop-blur-none shadow-2xl ${sidebarOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
                                ...
                                <div className="grid grid-cols-2 gap-2 p-1">
                                    {STATIC_ELEMENTS.map((el) => (
                                        <div
                                            ...
                                            className="flex flex-col items-center gap-2 p-3 rounded-xl bg-page hover:bg-page border border-transparent hover:border-border transition-all cursor-grab active:cursor-grabbing group"
                                        >
