...
export function MobileNav({ currentTab, onNavigate, onQuickAction }: MobileNavProps) {
    ...
    return (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[100] px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2 pointer-events-none">
            <div className="bg-black/80 backdrop-blur-2xl border border-white/5 rounded-[2rem] h-20 flex items-center justify-around px-2 shadow-2xl pointer-events-auto relative overflow-hidden max-w-[480px] mx-auto">
                {/* Subtle top glow */}
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            className="flex flex-col items-center gap-1 group relative overflow-hidden py-2"
                        >
                            {isActive && (
