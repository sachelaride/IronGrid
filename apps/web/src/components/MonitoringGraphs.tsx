...
import {
    ...
    Search, Cpu, HardDrive, BarChart3, SlidersHorizontal
} from 'lucide-react';
import {
...
const TrafficChart = ({ inData = [], outData = [] }: { inData: any[], outData: any[] }) => {
    ...
    const formatTraffic = (bitsPerSec: number) => {
    ...
    if (data.length < 2) return (
        <div className="h-full w-full flex items-center justify-center bg-card/90 border border-dashed border-border rounded-2xl">
            <span className="text-[10px] font-black text-main/90 uppercase italic">Coletando dados...</span>
        </div>
    );
                                ...
                                return (
                                    <div className="bg-card border border-border p-2 rounded-lg shadow-xl text-[10px]">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span className="text-main/90">Entrada: {formatTraffic(payload[0].value)}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <div className="w-1.5 h-1.5 rounded-full bg-purple-500" />
                                            <span className="text-main/90">Saída: {formatTraffic(payload[1].value)}</span>
                                        </div>
                                    </div>
                                );
                            }
...
export function MonitoringGraphs() {
    ...
    return (
        <div className="flex flex-col gap-6 h-full min-h-[700px] w-full">
            ...
            <div className="bg-card border border-border rounded-[2.5rem] p-4 sm:p-8 shadow-xl relative overflow-hidden group">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 sm:gap-8 relative z-10">
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-4 sm:gap-6 w-full lg:w-auto">
                        {/* Custom Dropdown/Combobox */}
                        <div className="relative w-full md:w-[650px] group/dropdown">
                            <div className="absolute -top-6 left-0 flex items-center gap-2">
                                <span className="text-[10px] font-black text-main/90 uppercase tracking-widest italic">Monitoramento de:</span>
                            </div>
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                <div className="relative flex-1">
                                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-main/80 z-20" />
                                <select
                                    ...
                                    className="w-full pl-12 pr-10 py-4 bg-card/80 border border-border rounded-2xl text-sm md:text-base font-black italic text-main appearance-none focus:outline-none focus:border-primary/50 transition-all cursor-pointer shadow-inner uppercase tracking-tight text-ellipsis overflow-hidden whitespace-nowrap"
                                >
                                ...
                                <ChevronRight className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-secondary/90 rotate-90 pointer-events-none" />
                                </div>
                                {selectedDevice && (
                                    <button
                                        onClick={() => setShowInterfaceConfig(true)}
                                        className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-2 px-5 py-4 bg-card/80 border border-border rounded-2xl text-main/90 hover:text-accent hover:border-accent/40 hover:bg-accent/5 transition-all active:scale-95 group font-black uppercase text-[10px] tracking-widest italic whitespace-nowrap"
                                    >
                                        <SlidersHorizontal className="w-4 h-4 group-hover:rotate-12 transition-transform shrink-0" />
                        ...
                        {selectedDevice && (
                            <div className="flex items-center gap-4 border-t sm:border-t-0 sm:border-l border-border pt-4 sm:pt-0 sm:pl-6 w-full sm:w-auto">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] font-mono font-bold text-main/80 mt-1 uppercase tracking-widest">
            ...
            <main className="flex-1 space-y-6">
                {selectedDevice ? (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
                        ...
                        {/* Interfaces Section */}
                        <div className="bg-card border border-border rounded-[2.5rem] p-4 sm:p-6 lg:p-10 shadow-xl">
                            <h3 className="text-xl font-black text-main mb-6 sm:mb-8 flex items-center gap-3 italic tracking-tight">
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                                {selectedDevice.interfaceDetails?.filter((i: any) => i.enabled === true).map((iface: any) => (
                                    <div
                                        ...
                                        className="bg-card/80 border border-border p-4 sm:p-6 rounded-[2rem] hover:border-primary/40 hover:bg-card transition-all cursor-pointer group flex flex-col gap-4 sm:gap-6"
                                    >
                                        <div className="flex justify-between items-start">
                                            <div className="flex flex-col min-w-0">
                                                <span className="font-black text-main/95 italic text-base uppercase group-hover:text-main transition-colors truncate">
                                                    {iface.alias || iface.name}
                                                </span>
                                                {iface.alias && iface.alias !== iface.name && (
                                                    <span className="text-[10px] text-main/90 font-black uppercase truncate mt-0.5 opacity-90 decoration-primary/30 underline-offset-2">
                                                        {iface.name}
                                                    </span>
                                                )}
                                                <span className="text-[10px] text-main/85 font-bold uppercase tracking-widest mt-1">
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 pt-4 border-t border-border mt-auto">
                                            <div className="flex gap-4 sm:gap-6">
                                                <div className="flex flex-col">
                                                    <span className="text-[8px] font-black text-main/80 uppercase tracking-widest mb-1">Entrada (Rx)</span>
                                                    <span className="text-xl font-mono font-black text-main italic">
                                                        {((iface.latest?.in || 0) / 1000 / 1000).toFixed(2)} <span className="text-[10px]">Mbps</span>
                                                    </span>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[8px] font-black text-main/80 uppercase tracking-widest mb-1">Saída (Tx)</span>
                                                    <span className="text-xl font-mono font-black text-accent italic">
                                                        {((iface.latest?.out || 0) / 1000 / 1000).toFixed(2)} <span className="text-[10px]">Mbps</span>
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col sm:items-end gap-2">
                                                ...
                                                <span className="text-[9px] font-black text-main/90 group-hover:translate-x-1 transition-transform uppercase italic flex items-center gap-1">
                                ...
                                {(!selectedDevice.interfaceDetails || selectedDevice.interfaceDetails.filter((i: any) => i.enabled === true).length === 0) && (
                                    <div className="col-span-full py-10 px-4 sm:px-10 bg-card/80 border border-dashed border-border rounded-[2rem] text-center space-y-4">
                                        <TrendingUp className="w-12 h-12 text-main/85 mx-auto" />
                                        <div>
                                            <p className="text-main/90 font-bold italic uppercase tracking-widest">Sem tráfego SNMP registrado</p>
                                            <p className="text-xs text-main/80 max-w-xs mx-auto mt-2 italic font-medium">Habilite o monitoramento de interfaces na aba "Devices" para habilitar estes gráficos.</p>
                                        </div>
                                    </div>
                                )}
                            </div>
            ...
            {showInterfaceConfig && selectedDevice && (
                <InterfaceConfigModal
                    device={selectedDevice}
                    onClose={() => setShowInterfaceConfig(false)}
                />
            )}
        </div>
    );
    ...
    return (
        <div className="bg-card border border-border p-8 rounded-[2rem] shadow-lg group hover:border-primary/30 transition-all">
            <div className="flex items-center gap-4 mb-3">
                ...
                <span className="text-[10px] font-black text-main/85 uppercase tracking-widest">{label}</span>
            </div>
