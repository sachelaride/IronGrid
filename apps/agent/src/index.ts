...
const client = createTRPCProxyClient<AppRouter>({
    links: [
        httpBatchLink({
            url: `${SERVER_URL}/trpc`,
            headers() {
                return config.ingestToken ? { 'x-agent-token': config.ingestToken } : {};
            },
        }),
    ],
});
...
async function startAgent() {
    ...
    if (args.includes('--install')) {
        ...
        const rustdeskArg = args.find(a => a.startsWith('--rustdesk='))?.split('=')[1];
        const rustdeskKeyArg = args.find(a => a.startsWith('--rustdesk-key='))?.split('=')[1];
        const rustdeskRelayArg = args.find(a => a.startsWith('--rustdesk-relay='))?.split('=')[1];
        await installAgent(serverUrlArg, rustdeskArg, rustdeskKeyArg, rustdeskRelayArg, args.includes('--skip-snmp-prompt'));
        process.exit(0);
    }
    ...
    console.log(`\x1b[36m⚡ \x1b[0m\x1b[36mVERSION: 12-ENTERPRISE (TRON CORE)\x1b[0m`);
    console.log(`\x1b[36m⚡ \x1b[0m\x1b[31mSERVER: ${SERVER_URL}\x1b[0m`);
    console.log(`\x1b[36m⚡ \x1b[0m\x1b[31m====================================================\x1b[0m`);

    // Obtém instância do Socket para comunicação persistente
    ...
    // Escuta pedido de forçar inventário
    ...
    // Loop de relatório de inventário (Novo Padrão: 4 horas)
    // 4h = 4 * 60 * 60 * 1000 = 14.400.000ms
    const FOUR_HOURS = 14400000;
    const inventoryInterval = (config as any).inventoryInterval || FOUR_HOURS;
    setInterval(async () => {
        await reportInventory();
    }, inventoryInterval);

    // Lógica de Atualização Forçada na Madrugada (Dawn Force Update)
    // Verifica a cada hora se estamos entre 2h e 4h da manhã para forçar inventário completo
    let lastDawnUpdate: string | null = null;
    setInterval(async () => {
        const now = new Date();
        const hour = now.getHours();
        const todayStr = now.toISOString().split('T')[0];

        // Se estiver entre 2 e 4 da manhã e ainda não rodou hoje
        if (hour >= 2 && hour <= 4 && lastDawnUpdate !== todayStr) {
            console.log('[Agent] Madrugada detectada. Forçando inventário completo...');
            await reportInventory(true);
            lastDawnUpdate = todayStr;
        }
