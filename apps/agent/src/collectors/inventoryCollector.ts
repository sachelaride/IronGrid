...
export class InventoryCollector {
    ...
    async collectFullInventory(): Promise<InventoryPayload> {
        ...
        const rustdeskInfo = await this.collectRustDeskInfo();
        ...
        return {
            ...
            peripherals,
            interfaces,
            rustdeskId: rustdeskInfo.id,
            rustdeskPassword: rustdeskInfo.password,
        };
    }
    ...
    async collectRustDeskInfo(): Promise<{ id?: string, password?: string }> {
        ...
        try {
            ...
            if (fs.existsSync(configPath)) {
                const configStr = await fsPromises.readFile(configPath, 'utf8');
                const config = JSON.parse(configStr);
                password = config.rustdeskPassword;
            }

            if (fs.existsSync(rustdeskPath)) {
                try {
                    const { stdout } = await execAsync(`"${rustdeskPath}" --get-id`);
                    if (stdout) {
                        // Extract alphanumeric ID just in case there's extra whitespace
                        const trimmed = stdout.trim();
                        if (trimmed) id = trimmed;
                    }
                } catch(e) {
                    console.log('[Inventory] Failed to get RustDesk ID:', e);
                }
            }
        } catch(e) { }
