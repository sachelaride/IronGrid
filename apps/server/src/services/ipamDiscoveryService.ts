...
import { AlertSeverity, AlertStatus, IPAddressStatus } from '@prisma/client';
...
type ArpEvidence = Map<string, Set<string>>;
...
function addArpEvidence(map: ArpEvidence, ip: string, mac?: string | null) {
    const normalized = normalizeMac(mac);
    if (!normalized || !/^\d+\.\d+\.\d+\.\d+$/.test(ip)) return;
    const current = map.get(ip) || new Set<string>();
    current.add(normalized);
    map.set(ip, current);
}

async function getLocalARPTable(): Promise<ArpEvidence> {
    const map: ArpEvidence = new Map();
    try {
...
/* ────────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────────── */
function snmpGetSubtree(host: string, community: string, oid: string, timeout = 3000): Promise<ArpEvidence> {
    return new Promise((resolve) => {
        const result: ArpEvidence = new Map();
        let resolved = false;
...
async function getSnmpARPTable(host: string, communities: string[]): Promise<ArpEvidence> {
...
export class IPAMDiscoveryService {
    ...
    static async discoverSubnet(subnetId: string): Promise<{ discovered: number; total: number }> {
        ...
        try {
            ...
            const snmpMacMap: ArpEvidence = new Map();
            ...
            const arpEvidence = IPAMDiscoveryService.mergeArpEvidence(snmpMacMap, localArpMap);

            for (const [ip, macs] of arpEvidence) {
                const mac = Array.from(macs)[0];
            ...
            for (const addr of addresses) {
                ...
                const observedMacs = new Set<string>(arpEvidence.get(addr.ip) || []);
                const previousMac = normalizeMac(addr.mac);
                if (previousMac) observedMacs.add(previousMac);
                const mac = result?.mac || Array.from(observedMacs)[0] || addr.mac || null;
                let deviceId = deviceIpMap.get(addr.ip) || addr.deviceId;

                if (observedMacs.size > 1) {
                    await IPAMDiscoveryService.raiseIPConflictAlert({
                        ip: addr.ip,
                        subnet: subnet.subnet,
                        subnetName: subnet.name,
                        vlan: subnet.vlan,
                        macs: Array.from(observedMacs),
                        deviceId
                    });
                }
     ...
     */
    private static async collectSNMPArp(targetMap: ArpEvidence) {
        ...
        for (let i = 0; i < devices.length; i += SNMP_CHUNK) {
            ...
            await Promise.all(chunk.map(async (d: any) => {
                ...
                for (const [ip, macs] of arpMap) {
                    for (const mac of macs) addArpEvidence(targetMap, ip, mac);
                }
            }));
        }
    ...
    private static mergeArpEvidence(...maps: ArpEvidence[]): ArpEvidence {
        const merged: ArpEvidence = new Map();
        for (const map of maps) {
            for (const [ip, macs] of map) {
                for (const mac of macs) addArpEvidence(merged, ip, mac);
            }
        }
        return merged;
    }

    private static async raiseIPConflictAlert(data: {
        ip: string;
        subnet: string;
        ...
        const title = `[IPAM] Conflito de IP detectado: ${data.ip}`;
        ...
        const message = [
            `O IP ${data.ip} foi visto com mais de um MAC durante a descoberta IPAM.`,
            `MACs observados: ${data.macs.join(', ')}`,
            `Sub-rede: ${data.subnetName || data.subnet}`,
            data.vlan ? `VLAN: ${data.vlan}` : null,
            'Ação sugerida: consultar a tabela MAC do switch para localizar as portas dos MACs envolvidos e corrigir endereço estático/DHCP.'
        ].filter(Boolean).join('\n');
