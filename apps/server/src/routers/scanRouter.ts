...
export const scanRouter = router({
    ...
    quickScan: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            ...
            const enrichmentPromises = results.map(async (device) => {
                ...
                let finalModel = null;
                let snmpCommunityId: string | undefined;

                if (snmpData) {
                    if (snmpData.community) {
                        const snmpCommunity = await prisma.snmpCommunity.findFirst({
                            where: { community: { equals: snmpData.community, mode: 'insensitive' } }
                        });
                        snmpCommunityId = snmpCommunity?.id;
                    }
                    if (snmpData.sysName) {
                ...
                const updateData = {
                    ...
                    lastSeen: new Date(),
                    ...(snmpCommunityId ? { snmpCommunityId } : {})
                };
    ...
    bulkUpdateDevices: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            ...
            if (data.snmpCommunityId) {
                const [community, devices] = await Promise.all([
                    prisma.snmpCommunity.findUnique({
                        where: { id: data.snmpCommunityId },
                ...
                const ips = devices.map(device => device.ipAddress).filter(Boolean) as string[];
                if (community?.community && ips.length > 0) {
                    await prisma.monitoredDevice.updateMany({
                        where: { ip: { in: ips } },
                        data: { community: community.community }
                    });
                }
            }
