...
import { AlertStatus, IPAddressStatus } from '@prisma/client';
...
export const ipamRouter = router({
    listSubnets: protectedProcedure.query(async () => {
    ...
    listIPConflicts: protectedProcedure.query(async () => {
        return prisma.alert.findMany({
            where: {
                status: AlertStatus.ACTIVE,
                title: {
                    startsWith: '[IPAM] Conflito de IP detectado:'
                }
            },
            include: {
                device: {
