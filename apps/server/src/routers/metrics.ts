...
import { escapeFluxString, fluxEqualsAny } from '../utils/flux';
import { TRPCError } from '@trpc/server';
import { env } from '../utils/env';
import { assertCanCreateAsset } from '../utils/productEdition';

let warnedMissingAgentToken = false;

function validateAgentIngest(ctx: any) {
    const expectedToken = env.AGENT_INGEST_TOKEN?.trim();

    if (!expectedToken) {
        if (!warnedMissingAgentToken) {
            Logger.error('AGENT_INGEST_TOKEN is not configured; metrics ingest is running in compatibility mode.');
            warnedMissingAgentToken = true;
        }
        return;
    }

    const suppliedToken = String(ctx.req?.headers?.['x-agent-token'] || '').trim();
    if (suppliedToken !== expectedToken) {
        throw new TRPCError({ code: 'UNAUTHORIZED', message: 'Token de agente invalido.' });
    }
}
        ...
        .query(async ({ input, ctx }) => {
            ...
            const query = `
                from(bucket: "${influxDB.bucket}")
                    ...
                    |> filter(fn: (r) => r["device_id"] == "${escapeFluxString(filterId)}" or r["device"] == "${escapeFluxString(filterId)}")
    ...
    getInterfaceMetrics: protectedProcedure
        ...
        .query(async ({ input }) => {
            ...
            if (interfaceIndex !== 'all') {
                ...
                const orClause = fluxEqualsAny(['interface_index', 'interface'], candidates);
            ...
            const query = `
                from(bucket: "${influxDB.bucket}")
                    ...
                    |> filter(fn: (r) => r["device"] == "${escapeFluxString(filterId)}" or r["device_id"] == "${escapeFluxString(filterId)}")
