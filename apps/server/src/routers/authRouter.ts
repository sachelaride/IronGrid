...
export const authRouter = router({
    ...
    login: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (user.twoFactorEnabled && user.twoFactorSecret) {
                if (!input.totpToken) {
                    console.log(`[AUTH] 2FA required for: ${input.username}`);
                    return { requires2FA: true };
                }
                
                const isValid = authenticator.verify({ token: input.totpToken, secret: user.twoFactorSecret });
                if (!isValid) {
                    throw new TRPCError({
                        code: 'UNAUTHORIZED',
     ...
     * Generate 2FA Secret
     * 
     * Gera um segredo TOTP e a URL do QR Code para o usuário configurar em seu app autenticador.
     */
    generate2FA: protectedProcedure.mutation(async ({ ctx }) => {
        const user = await (prisma as any).user.findUnique({ where: { id: ctx.user!.id } });
        if (!user) throw new TRPCError({ code: 'NOT_FOUND' });

        const secret = authenticator.generateSecret();
        const otpauth = authenticator.keyuri(user.email || user.username, 'IronGrid', secret);
     ...
     * Enable 2FA
     * 
     * Valida um token TOTP com um segredo gerado e habilita o 2FA para o usuário.
     */
    enable2FA: protectedProcedure
        .input(z.object({ token: z.string(), secret: z.string() }))
        .mutation(async ({ input, ctx }) => {
            const isValid = authenticator.verify({ token: input.token, secret: input.secret });
            if (!isValid) {
                throw new TRPCError({ code: 'BAD_REQUEST', message: 'Token TOTP inválido.' });
            }

            await (prisma as any).user.update({
                where: { id: ctx.user!.id },
                data: { twoFactorSecret: input.secret, twoFactorEnabled: true }
            });
     ...
     * Disable 2FA
     * 
     * Desabilita a autenticação em dois fatores, requerendo validação do token atual.
     */
    disable2FA: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (!user || !user.twoFactorEnabled || !user.twoFactorSecret) {
                throw new TRPCError({ code: 'BAD_REQUEST', message: '2FA não está ativado.' });
            }
