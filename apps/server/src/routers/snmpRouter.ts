...
import { fluxEqualsAny } from '../utils/flux';
import { TRPCError } from '@trpc/server';
import { resolveInterface } from '../utils/network';
import { assertCanEnableGrafanaCharts, enforceGrafanaChartLimit } from '../utils/productEdition';
...
export async function syncMonitoredDevices() {
    try {
        ...
        const resolveCommunity = (device: any, fallback: string) => device?.snmpCommunity?.community || fallback;
        ...
        const processed = await Promise.all(devices.map(async d => {
            if (!d.interfaces || d.interfaces.length === 0) {
                const device = d.deviceId 
                    ? await prisma.device.findUnique({ where: { id: d.deviceId }, include: { networkInterfaces: true, snmpCommunity: true } })
                    : await prisma.device.findFirst({
                        where: { ipAddress: d.ip },
                        include: { networkInterfaces: true, snmpCommunity: true }
                    });
                ...
                if (ids.length === 0) {
                    ...
                    try {
                        const probeCommunity = resolveCommunity(device, d.community);
                        const probe = await snmpService.probeDevice(d.ip, [probeCommunity]);
                        if (probe && probe.interfaces.length > 0) {
                            ...
                            const dbDevice = await prisma.device.findFirst({ where: { ipAddress: d.ip }, include: { snmpCommunity: true } });
                        ...
                        const isLag = (i: any) => {
                            ...
                            ids = probe.interfaces.filter(i => !isLag(i)).map(i => i.index);
                            }
                        }
                    } catch (e) {
                ...
                if (ids.length > 0) {
                    const deviceLookup = d.deviceId 
                        ? await prisma.device.findUnique({ where: { id: d.deviceId }, include: { snmpCommunity: true } })
                        : await prisma.device.findFirst({ where: { ipAddress: d.ip }, include: { snmpCommunity: true } });
                    
                    const allIfaces = await prisma.networkInterface.findMany({
                        where: { deviceId: deviceLookup?.id }
                    });

                    const enabledIds = allIfaces
                        .filter(ni => ((ni as any).enabled === true || (ni as any).autoEnabled === true) && Number.isInteger(ni.index) && ni.index > 0)
                        .map(ni => ni.index);
                    const desiredCommunity = resolveCommunity(deviceLookup, resolveCommunity(device, d.community));

                    await prisma.monitoredDevice.update({
                        where: { id: d.id },
                        data: {
                            interfaces: enabledIds,
                ...
                const dbDevice = d.deviceId
                    ? await prisma.device.findUnique({ where: { id: d.deviceId }, include: { networkInterfaces: true, snmpCommunity: true } })
                    : await prisma.device.findFirst({ where: { ipAddress: d.ip }, include: { networkInterfaces: true, snmpCommunity: true } });
                
                if (dbDevice && dbDevice.networkInterfaces.length > 0) {
                    const enabledIds = dbDevice.networkInterfaces
                        .filter(ni => ((ni as any).enabled === true || (ni as any).autoEnabled === true) && Number.isInteger(ni.index) && ni.index > 0)
                        .map(ni => ni.index);
                    const desiredCommunity = resolveCommunity(dbDevice, d.community);

                    if (JSON.stringify(d.interfaces.sort()) !== JSON.stringify(enabledIds.sort()) || d.community !== desiredCommunity) {
                        await prisma.monitoredDevice.update({
                            where: { id: d.id },
                            data: { interfaces: enabledIds, community: desiredCommunity }
                        });
                        return { ...d, interfaces: enabledIds, community: desiredCommunity };
                    }
                }
            }
            return d;
        ...
        monitoredDevices.splice(0, monitoredDevices.length, ...processed.map(d => ({
            ...
            interfaces: (d.interfaces || []).filter((index: number) => Number.isInteger(index) && index > 0)
        })));
...
export const snmpRouter = router({
    ...
    listMonitoredDevices: protectedProcedure.query(async () => {
        ...
        const enriched = await asyncPool(monitored, 10, async (m) => {
            ...
            const filterClause = fluxEqualsAny(['device_id', 'device'], filterIds);
            ...
            const dbInterfaces = resolvedDevice.networkInterfaces
                .filter((ni: any) => Number.isInteger(ni.index) && ni.index > 0)
                .map((ni: any) => ({
                index: ni.index,
    ...
    toggleInterface: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            if (input.index <= 0) {
                throw new TRPCError({
                    code: 'BAD_REQUEST',
                    message: 'Interface SNMP invalida: ifIndex deve ser maior que zero.',
                });
            }
            ...
            if (input.enabled) {
                const existing = await prisma.networkInterface.findFirst({
                    ...
                    select: { enabled: true, autoEnabled: true },
                });
                if (!existing?.enabled && !existing?.autoEnabled) {
                    await assertCanEnableGrafanaCharts(1);
                }
            }
    ...
    bulkToggleInterfaces: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            ...
            if (input.enabled) {
                const newlyEnabled = await prisma.networkInterface.count({
                    where: {
                        ...where,
                        NOT: {
                            OR: [
                ...
                await assertCanEnableGrafanaCharts(newlyEnabled);
            }
