...
export const customMapRouter = router({
    ...
    getPortBandwidth: protectedProcedure
        ...
        .query(async ({ input }) => {
            ...
            const deviceFilter = fluxEqualsAny(['device', 'device_id'], filterIds);
            ...
            const ifaceFilter = fluxEqualsAny(['interface', 'interface_index'], Array.from(ifaceFilterIds));
    ...
    getDeviceInterfaces: protectedProcedure
        ...
        .query(async ({ input }) => {
            ...
            if (device.agentId) filterIds.push(device.agentId);
            if (device.name) filterIds.push(device.name);
            const filterClause = fluxEqualsAny(['device', 'device_id'], filterIds);
